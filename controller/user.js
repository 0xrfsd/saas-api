...
exports.mailLogin = async (req, res) => {
  try {
...
exports.phoneLogin = async (req, res) => {
  try {
    ...
    const user = await Usuario.findOne({ phone }).lean();

    if (phone.length == 0) {
      return res.json({ status: "Erro!", error: "Qual é o seu telefone?" });
    }
...
exports.omniLogin = async (req, res) => {
  try {
    const { verifier, senha } = req.body;

    const user = await Usuario.findOne({ verifier }).lean();

    if (verifier.length == 0) {
      return res.json({
        status: "Erro!",
        error: "Qual o seu email ou telefone?",
      });
    }
